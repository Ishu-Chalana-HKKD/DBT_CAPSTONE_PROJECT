with boardgames_filtered as (
    select * from {{ ref('int_boardgames_boardgames_filtered') }}
),

dim_boardgames as (
    select 
        {{ dbt_utils.generate_surrogate_key(['boardgame_id']) }} as boardgame_key,
        boardgame_id,
        boardgame_name,
    ...
    from {{ ref('int_boardgames_boardgames_filtered') }}
)
